...
main(argl : list of string)
{
	...
	sys->bind("#sacmeplumb", "/chan", Sys->MREPL|Sys->MCREATE);
	wdir = workdir->init();
	if (wdir == nil)
		wdir = ".";
	workdir = nil;
	...
	spawn exec->run(nil, "{bind -c '#sacmeplumb' /chan; plumber /acme/plumbing > /tmp/plumb.log >[2=1]&}", nil, 0, TRUE, nil, nil, FALSE);
	spawn plumbproc();
